...
import { Bell, MessageSquare, ShieldCheck, ExternalLink, Settings2, Users } from "lucide-react";
...
const teamCoreRoles: { role: UserRole, description: string, emphasis?: boolean }[] = [
    { role: 'guest', description: 'New user, awaiting verification to access most features.', emphasis: false },
    { role: 'verified', description: 'Verified user, can participate and contribute to assigned projects.', emphasis: false },
    { role: 'Developer', description: 'Focuses on coding, system architecture, and technical implementation.', emphasis: false },
    { role: 'Builder', description: 'Concentrates on creating assets, environments, or infrastructure components.', emphasis: false },
    { role: 'Designer', description: 'Works on UI/UX, visual assets, and overall aesthetic.', emphasis: false },
    { role: 'Community Manager', description: 'Engages with the user base, gathers feedback, and manages community platforms.', emphasis: false },
    { role: 'Moderator', description: 'Helps maintain order, enforce guidelines, and manage content on platforms.', emphasis: false },
    { role: 'Project Manager', description: 'Oversees specific projects, manages timelines, resources, and team coordination.', emphasis: true },
    { role: 'admin', description: 'Manages users, roles, site-wide settings, and has access to admin tools.', emphasis: true },
    { role: 'owner', description: 'Overall platform lead with full administrative rights and strategic oversight.', emphasis: true },
];
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your connections, notifications, and understand your role within TeamCore.</CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card className="shadow-sm hover:shadow-md transition-shadow">
        <CardHeader className="flex flex-row items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>Control how you receive updates from TeamCore.</CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-md border">
            <div>
              <Label htmlFor="community-activity" className="font-semibold">Team Activity</Label>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">
              Our community forums are the central place for all TeamCore discussions.
              (UI Placeholder)
            </p>
        ...
        <Card className="shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">
              Link your TeamCore account with Discord for instant notifications. (Feature not implemented)
            </p>
            <Button disabled>
      ...
      <Card className="shadow-sm hover:shadow-md transition-shadow">
        <CardHeader className="flex flex-row items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>Understand roles and their typical responsibilities within TeamCore.</CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Roles in TeamCore define access levels and responsibilities. Progression is managed by Admins/Owners.
          </p>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {teamCoreRoles.map(item => (
                 <div key={item.role} className={`p-4 border rounded-md ${item.emphasis ? 'bg-primary/10 border-primary/30' : 'bg-muted/50'}`}>
                    <h4 className={`font-semibold ${item.emphasis ? 'text-primary' : ''}`}>{item.role.charAt(0).toUpperCase() + item.role.slice(1)}</h4>
                    <p className="text-xs text-muted-foreground mt-1">{item.description}</p>
                </div>
            ))}
          </div>
           <p className="text-sm text-muted-foreground">
            Specific permissions are managed by Admins/Owners. Details will be available on the (upcoming) Forums or Wiki.
          </p>
        </CardContent>
      </Card>
    </div>
